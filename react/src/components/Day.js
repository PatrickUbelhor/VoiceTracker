...
function Day(props) {
	...
	let markers = [<span key={'label0'} className="label" style={{ left: '-0.9em' }}>0:00</span>];
	const numMarkers = 12;
	for (let i = 1; i < numMarkers; i++) {
		const left = (i * 100 / numMarkers);
		const negativeOffset = (i < 5) ? -0.75 : -1.25; // Used to align label to marker. Longer times require more offset (4:00 vs 14:00).
		markers.push(
			// NOTE: The key of the label and div should be named after the time
